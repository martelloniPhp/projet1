...
MultiComponent* BaseModel::toMulti(BaseModel* model)
{
    if (model and model->isMulti()) {
        return static_cast<MultiComponent*>(model);
    }
    return nullptr;
}
                               ...
                               AtomicModelVector& list, MulticomponentVector& list2,ComponentVector& list3)
{
    ...
    if (model->isAtomic()) {
		...
		list3.push_back((Component*)model);
	}
	else {
                               ...
                               ComponentVector& list)
{
	...
	coupledModelList.push_back((CoupledModel*)model);
		...
		BaseModel* n = elem.second;
                if (n->isMulti()) {
                    list.push_back(static_cast < Component*>(n));
                } else {
                    coupledModelList.push_back(static_cast < CoupledModel*>(n));
                }
	    }
