...
void SaxStackVpz::pushModel(const xmlChar **att)
{
    ...
    vpz::MultiComponent *container = nullptr;
    ...
    if (type == nullptr) {
     ...
     if (xmlStrcmp(type, (const xmlChar *)"atomic") == 0) {
        ...
        try {
            ...
            gmdl = ptr;
            container = ptr;
          //  std::cout <<"multi parent: "<< cplparent->getName() << " container= "<< container->getName() << std::endl;
        }
        catch (const utils::DevsGraphError &e) {
            throw utils::SaxParserError(
                (fmt(_("Error build multi model '%1%' with error: %2%")) %
                 name % e.what())
                    .str());
        }
    }
        ...
        throw utils::SaxParserError(
            (fmt(_("Unknow modele type %1%")) % (const char *)type).str());
    }
