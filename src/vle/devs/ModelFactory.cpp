 ...
 std::cout << "atomicModelList: "<< atomicmodellist.size()<< "  multicomponentlist: "<< multicomponentlist.size() << "  Componentlist: "<< componentlist.size()<< std::endl;
        for (auto &elem : atomicmodellist) {
            createModel(coordinator,
                        ...
                        (elem)->conditions(),
                        (elem)->observables());
        if(elem->get_simulator()->dynamics() == nullptr)
        {
        std::cout << "atomic dyn null"<< std::endl;
		}else {
		std::cout << "atomic dyn non null"<< std::endl;
		}
        }
        ...
        if(elem->get_simulator() == nullptr)
        {   
            std::cout << "Milticomposent: name = " << elem->getName() << " components= " << elem->getComponents().size() << " dynamics= null "<<std::endl;
        }else{
			 std::cout << "Milticomposent: name = " << elem->getName() << " components= " << elem->getComponents().size() << " dynamics= " << elem->get_simulator()->nbDynamics() << std::endl;
		}
		for (auto &comp: elem->getComponents())
		{
			std::cout << "composent = " << comp->getCompleteName() << " dynamics = "<< comp->dynamics() << std::endl;
		}
        }
        /*
...
std::unique_ptr<DynamicsComp> buildDynamics(utils::ContextPtr context,
                                           ...
                                           void *symbol)
{
    typedef DynamicsComp *(*fctdyn)(const DynamicsInit &, const InitEventList &); 
