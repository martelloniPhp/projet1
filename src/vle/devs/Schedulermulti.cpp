...
void SchedulerMulti::init(Time time)
{
    m_current_time = time;
    ...
    while (not m_scheduler.empty() and
        ...
        Dynamics *dyn = m_scheduler.top().m_dynamic;
        ...
        //
            ...
            m_current_bag.dynamics.emplace_back(dyn);

        m_current_bag.unique_dynamics.emplace(sim);

        dyn->setInternalEvent();
        dyn->resetHandle();
        m_scheduler.pop();
    }
}

void Scheduler::addInternal(Dynamics *dynamic, Time time)
{
